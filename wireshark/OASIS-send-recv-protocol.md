...
### 10. PCAP Capturing Notes

For analysis, packet captures (PCAP files) can be generated by utilities implementing this protocol (e.g., `oasis-utils`). These captures often use a specific link-layer type (e.g., `LINKTYPE_USER2 (149)`).

A key aspect for interpreting these captures is that the capturing utility (`oasis_send` or `oasis_recv`) typically prepends a **direction byte** to the actual serial data for each frame in the PCAP file:
...
The actual OASIS protocol message (e.g., starting DLE STX or ENQ) follows this direction byte in the captured frame.
