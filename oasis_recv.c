...
int main(int argc, char *argv[]) {
    ...
    for (int retries = 100;; retries--) {
        if (read_serial(fd, commBuffer, 1) == 1) {
            if (commBuffer[0] == ENQ) {
    ...
    while (1) {
        ...
        if (current_segment == debug_segment) dump_hex(commBuffer,
                                                       (int)bytes_read);

        if (bytes_read > 3) {
            cksum = oasis_packet_decode(commBuffer,
                                        (uint16_t)bytes_read,
                                        decoded_buf,
                                        &decoded_len);
            ...
            if (cksum) {
                ...
                switch (pHdr->rectype) {
                    ...
                    case WRITE: /* Data Packet */
                    {
                        ...
                        if (current_segment == debug_segment) {
                            dump_hex(decoded_buf, decoded_len);
                        }

                        if ((pDirEntry->file_format & 0x1F) !=
                            FILE_FORMAT_SEQUENTIAL) {
                            current_segment++;
    ...
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] != '-') {
            switch (positional_arg_cnt) {
                case 0:
                    snprintf(args->port_path,
                             sizeof(args->port_path),
                             "%s",
                             argv[i]);
                    break;
                case 1:
...
int oasis_open_file(directory_entry_block_t *dir_entry, FILE **ostream, const char *path, int quiet) {
    ...
    switch (dir_entry->file_format & 0x1f) {
        ...
        case FILE_FORMAT_SEQUENTIAL:
            file_len =
                (dir_entry->file_format_dependent2 + 1 -
    ...
    if (!quiet) {
        printf("Receiving \"%s.%s\" -> %s (%d bytes)\n",
               oasis_fname,
               oasis_ftype,
               output_filename,
               file_len);
